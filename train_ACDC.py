...
parser.add_argument("--batch_size", default=8, help="batch size") #12
parser.add_argument("--lr", default=0.0001, help="learning rate")
parser.add_argument("--max_epochs", default=25)
parser.add_argument("--n_skip",default=5)
parser.add_argument("--img_size", default=256)
parser.add_argument("--save_path", default="./model_pth/ACDC")
parser.add_argument("--n_gpu", default=1)
parser.add_argument("--checkpoint", default=None)
parser.add_argument("--list_dir", default="./DataProcessed/lists_ACDC") # change it to the path where your processed data is stored for the lists_acdc
parser.add_argument("--root_dir", default="./DataProcessed/")# change it to the path where your processed data is stored
parser.add_argument("--volume_path", default="./DataProcessed/test") # change the data to where the processed data is stored for test
...
snapshot_path = "{}/{}/{}".format(args.save_path, args.exp, 'MERIT_Cascaded_Small_loss_MUTATION_w3_7')  #MERIT_Cascaded_Small_loss_MUTATION_w3_7
...
def val():
    ...
    for i, val_sampled_batch in enumerate(valloader):
        ...
        if x != args.img_size or y != args.img_size:
            ...
            val_label_batch_changed = zoom(val_label_batch, (args.img_size / x, args.img_size / y), order=0) # Resize labels to match the images 
        val_image_batch = torch.from_numpy(val_image_batch).unsqueeze(0).unsqueeze(0).float().cuda()
        val_label_batch_changed = torch.from_numpy(val_label_batch_changed).unsqueeze(0).long().cuda() # Convert labels to torch tensor
        ...
        for s in ss:
            ...
            for idx in range(len(s)):
                iout += P[s[idx]]
            loss_ce = ce_loss(iout, val_label_batch_changed)
            loss_dice = dice_loss(iout, val_label_batch_changed, softmax=True)
            val_loss += (lc1 * loss_ce + lc2 * loss_dice)
        ...
        # trying validation loss
        

        dc_sum+=dc(val_outputs,val_label_batch[:])
    performance = dc_sum / len(valloader)
    val_loss_avg = val_loss_sum / len(valloader)
    logging.info('Testing performance in val model: mean_dice : %f, best_dice : %f, loss:%f' % (performance, Best_dcs,val_loss_avg))

    print('Testing performance in val model: mean_dice : %f, best_dice : %f, loss:%f' % (performance, Best_dcs,val_loss_avg))
    #print("val avg_dsc: %f" % (performance))
    return performance, val_loss_avg

# Initialize val_loss before starting the training loop
val_loss = float('inf')
...
for epoch in iterator:
    net.train()
    train_loss_sum = 0.0  # Initialize cumulative training loss for the epoch
    ...
    avg_train_loss = train_loss_sum / len(train_loader)  # Compute average training loss for the epoch
    Loss.append(avg_train_loss)
    logging.info('Epoch %d : train_loss : %f' % (epoch, avg_train_loss))
    print('Epoch %d : train_loss : %f' % (epoch, avg_train_loss))
    
    save_model_path = os.path.join(snapshot_path, 'last.pth')
    torch.save(net.state_dict(), save_model_path)
    
    avg_dcs, val_loss = val()
        
    logging.info('Epoch %d : val_loss : %f' % (epoch, val_loss))
    ...
    if avg_dcs > Best_dcs_th or avg_dcs >= Best_dcs:
        avg_test_dcs, avg_hd, avg_jacard = inference(args, net, testloader, args.test_save_dir)
